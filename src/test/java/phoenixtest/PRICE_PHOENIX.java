...
public class PRICE_PHOENIX {
	...
	public static void main(String[] args) throws Exception {
		...
		PrintWriter p = new PrintWriter("/home/ec2-user/hbase_mongo_test/PRICE_PHOENIX.csv");
		p.println("UpdateCount,UpdatedDocument,Time_fetch_key(milisec),Time_update_key(milisec)");

		for (int i = 0; i < loopcount; i++) {

			String security = factory.getRandomSecurity();
			int value = random.get();

			//***********************//
			// fetching keys
			keyfetch.start();
			ArrayList<String> pk = dml.getkeys_SecurityCode(security);
			long keyfetchtime = keyfetch.stop();

			// updating keys
			keyupdate.start();
			for (String temp : pk) {
				dml.update_MarketPriceBase(temp, value);
			}
			...
			System.out.println("PRICE[cycle] " + i);
